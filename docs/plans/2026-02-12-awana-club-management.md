...
**Step 1: Write migration for clubs and memberships**
...
migrate((app) => {
  const clubs = new Collection({
    ...
    listRule: "@request.auth.id != '' && @collection.club_memberships.user.id ?= @request.auth.id",
    viewRule: "@request.auth.id != '' && @collection.club_memberships.user.id ?= @request.auth.id",
  });
  ...
  app.saveCollection(clubs);
  app.saveCollection(memberships);
  app.saveCollection(programs);
}, (app) => {
...
git commit -m "db: add core club management collections"
```
